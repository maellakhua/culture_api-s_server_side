...
class Europeana extends CI_Model {
    ...
    public function api($town, $lat, $long, $categories){
        if ($categories != NULL) { 
            ...
            $rlat = round($lat);
            $rlong = round($long);
            $info = file_get_contents('http://europeana.eu/api/v2/search.json?wskey=qaVVXQUY6&query=pl_wgs84_pos_lat:['.$rlat.'+TO+'.($rlat+1).']+AND+pl_wgs84_pos_long:['.$rlong.'+TO+'.($rlong+1).'].&profile=minimal');
            $decoded_info = json_decode($info);
            $items = $decoded_info->items;
            $finaljson = '{"results":';
            $final_array = array();
